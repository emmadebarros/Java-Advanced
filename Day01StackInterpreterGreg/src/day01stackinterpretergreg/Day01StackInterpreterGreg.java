package day01stackinterpretergreg;
...
/**
 *
 * @author emmadebarros / Teacher's version / Not complete : Missing fah2cel.txt (in Java3 notes Day01 homework)
 */
public class Day01StackInterpreterGreg {

    static Stack<Double> stack = new Stack<>();

    static Scanner input = new Scanner(System.in); // Scanner object connected to the keyboard

    static String chooseProgramFile() {
        System.out.println("These are *.txt files in the current directory");
        File dir = new File("."); // the . means everything is the current directory
    ...
    public static void main(String[] args) {
        ...
        String line = "";
        try (Scanner inputProg = new Scanner(progFile)) { // Scanner object to read
            while (inputProg.hasNextLine()) {
                line = inputProg.nextLine();
                String[] data = line.split(":");
                double val1, val2, res0; // declare 3 variables
                switch (data[0]) {
                    case "read":
